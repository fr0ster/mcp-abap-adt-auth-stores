/**
 * Tests for BtpSessionStore - broker usage scenarios
...
describe('BtpSessionStore - Broker Usage', () => {
  let tempDir: string;
  let store: BtpSessionStore;
  const testDestination = 'test-destination';

  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'btp-session-test-'));
    store = new BtpSessionStore(tempDir, 'https://default.mcp.com', createTestLogger());
  });
  ...
  describe('setConnectionConfig - new session', () => {
    ...
    it('should use defaultServiceUrl from constructor when serviceUrl is not provided', async () => {
      ...
      const storeWithDefault = new BtpSessionStore(tempDir, defaultServiceUrl, createTestLogger());
  ...
  describe('Broker flow simulation', () => {
    ...
    it('should handle broker flow: setAuthorizationConfig then setConnectionConfig', async () => {
      // BTP allows setting authorization config first
      const authConfig: IAuthorizationConfig = {
        uaaUrl: 'https://test.uaa.com',
        uaaClientId: 'test-client-id',
        uaaClientSecret: 'test-client-secret',
