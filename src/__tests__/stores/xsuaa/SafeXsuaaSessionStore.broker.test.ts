/**
 * Tests for SafeXsuaaSessionStore - broker usage scenarios
...
describe('SafeXsuaaSessionStore - Broker Usage', () => {
  let store: SafeXsuaaSessionStore;
  const testDestination = 'test-destination';

  beforeEach(() => {
    store = new SafeXsuaaSessionStore('https://default.mcp.com', createTestLogger());
  });
  ...
  describe('setConnectionConfig - new session', () => {
    ...
    it('should use defaultServiceUrl from constructor when serviceUrl is not provided', async () => {
      ...
      const storeWithDefault = new SafeXsuaaSessionStore(defaultServiceUrl, createTestLogger());
